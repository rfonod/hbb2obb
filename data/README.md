...
- **labels_hbb**: the horizontal boxes a detector found in the images, with its confidence in the 6th column. Nothing here was drawn or corrected by hand.
...
`--confidence_source combined` makes each score the conversion quality times the detector confidence from step 1. Drop the flag for the default `conversion` score alone, or pass `detector` for the input confidence alone; all three work on this data, since step 1 wrote that 6th column.
